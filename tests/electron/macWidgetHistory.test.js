...
// The restored 0.37 shell keeps one history resolver, `getDashboardHistory`,
// for both the dashboard and the native widget. Do not reintroduce the 0.45
// history-source UI module merely to service the widget pipeline.
test('the widget history adapter follows the restored dashboard resolver', () => {
  const fs = require('node:fs');
  const path = require('node:path');
  const main = fs.readFileSync(path.join(__dirname, '..', '..', 'src', 'electron', 'main.js'), 'utf8');
  const start = main.indexOf('async function macWidgetHistory()');
  assert.notEqual(start, -1);
  const body = main.slice(start, main.indexOf('\nfunction ', start + 1));
  assert.match(body, /settings\?\.historyEnabled === false/);
  assert.match(body, /return getDashboardHistory\(\);/);
  assert.doesNotMatch(main, /require\('\.\/historySource'\)/);
});
