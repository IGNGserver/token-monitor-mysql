...
function parseGraphResult(raw) {
  ...
  for (const row of rows) {
    ...
    // Component fields are optional on the legacy graph contract. Keep them
    // available to the richer history pipeline without changing the
    // enumerable shape consumed by older widgets when the source did not
    // explicitly advertise component provenance.
    const hasExplicitComponents = Boolean(row.tokenComponentSummary)
      || clientRows.some((entry) => entry?.tokenComponentsAvailable === true
        || Object.prototype.hasOwnProperty.call(entry || {}, 'unclassifiedTokens')
        || Object.prototype.hasOwnProperty.call(entry || {}, 'unclassified_tokens'));
    if (!hasExplicitComponents) {
      for (const key of ['cacheReadTokens', 'cacheWriteTokens', 'outputTokens', 'unclassifiedTokens', 'tokenComponentsAvailable']) {
        Object.defineProperty(contribution, key, { value: contribution[key], enumerable: false, configurable: true });
      }
      for (const value of Object.values(perClient).concat(Object.values(perModel))) {
        for (const key of ['cacheReadTokens', 'cacheWriteTokens', 'outputTokens', 'unclassifiedTokens']) {
          if (Object.prototype.hasOwnProperty.call(value, key)) {
            Object.defineProperty(value, key, { value: value[key], enumerable: false, configurable: true });
          }
        }
      }
    }
